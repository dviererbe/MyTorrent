...
namespace MyTorrent.DistributionServices.Mqtt.Tests.Events
{
    public class FragmentDistributionDeliveredEventTests
    {
        ...
        public FragmentDistributionDeliveredEventTests(ITestOutputHelper output)
        {
            Output = output;
            Example = new FragmentDistributionDeliveredEvent(
        ...
        {
            FragmentDistributionDeliveredEvent fragmentDistributionDeliveredEvent = FragmentDistributionDeliveredEvent.FromJsonString(JsonString_WithJsonSerializerOptions_WithIgnoreNullValues, SerializationTests.JsonSerializerOptions_WithIgnoreNullValues);

            Assert.Equal(Example.EventId, fragmentDistributionDeliveredEvent.EventId);
            Assert.Equal(Example.Hash, fragmentDistributionDeliveredEvent.Hash);
            Assert.Equal(Example.Receivers, fragmentDistributionDeliveredEvent.Receivers);
            Assert.Equal(Example.Data, fragmentDistributionDeliveredEvent.Data);
        }

        [Fact]
        public void FromJsonString_Should_DeserializeJsonStringCorrectly_WithJsonSerializerOptions_WithoutIgnoreNullValues()
        {
            FragmentDistributionDeliveredEvent fragmentDistributionDeliveredEvent = FragmentDistributionDeliveredEvent.FromJsonString(JsonString_WithJsonSerializerOptions_WithoutIgnoreNullValues, SerializationTests.JsonSerializerOptions_WithoutIgnoreNullValues);
